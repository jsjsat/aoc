...
function processBeamDownwards(beam: { x: number; y: number; value: number }, lines: string[]): {x: number; y: number; value: number}[] {
...
function processBeamsUpwards(lines: string[]): string {
    const grid: number[][] = Array(lines.length).fill(0).map(() => Array(lines[0].length).fill(0));
