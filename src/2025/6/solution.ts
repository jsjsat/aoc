...
const solution = {
  ...
  compute2(): string {
    let result = 0;
    const lines = parseLines(input);
    const nums = normalizeStrings(lines.slice(0, lines.length - 1)).map(line =>
      line.split(' ').map(num => num.padStart(4, '0'))
    );
    const ops = lines[lines.length - 1].split(' ').filter(op => op === '+' || op === '*');

    for (let i = 0; i < ops.length; i++) {
      let num0 = Number(
        (nums[0][i][0] != '0' ? nums[0][i][0] : '') +
          (nums[1][i][0] != '0' ? nums[1][i][0] : '') +
          (nums[2][i][0] != '0' ? nums[2][i][0] : '') +
          (nums[3][i][0] != '0' ? nums[3][i][0] : '')
      );
      let num1 = Number(
        (nums[0][i][1] != '0' ? nums[0][i][1] : '') +
          (nums[1][i][1] != '0' ? nums[1][i][1] : '') +
          (nums[2][i][1] != '0' ? nums[2][i][1] : '') +
          (nums[3][i][1] != '0' ? nums[3][i][1] : '')
      );
      let num2 = Number(
        (nums[0][i][2] != '0' ? nums[0][i][2] : '') +
          (nums[1][i][2] != '0' ? nums[1][i][2] : '') +
          (nums[2][i][2] != '0' ? nums[2][i][2] : '') +
          (nums[3][i][2] != '0' ? nums[3][i][2] : '')
      );
      let num3 = Number(
        (nums[0][i][3] != '0' ? nums[0][i][3] : '') +
          (nums[1][i][3] != '0' ? nums[1][i][3] : '') +
          (nums[2][i][3] != '0' ? nums[2][i][3] : '') +
          (nums[3][i][3] != '0' ? nums[3][i][3] : '')
      );
