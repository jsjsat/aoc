...
export default {
  compute1() {
    const file = readInput(2024, 7);
    let equations = file.split('\n').slice(0, -1);
    return (
      '' +
      equations
        .filter(eq => isValid1(eq))
        .map(eq => +eq.split(':')[0])
...
function validate1(testResult: number, operants: number[], cur: number): boolean {
  ...
  return (
    validate1(testResult, [...operants], cur + operant) ||
    validate1(testResult, [...operants], cur * operant)
  );
}
...
function validate2(testResult: number, operants: number[], cur: number): boolean {
  ...
  return (
    validate2(testResult, [...operants], cur + operant) ||
    validate2(testResult, [...operants], cur * operant) ||
    validate2(testResult, [...operants], +(cur + '' + operant))
  );
}
