...
export default {
  compute1() {
    let lines = readInput(2024, 12).split('\n').slice(0, -1);
    ...
    garden.forEach((plot, key) => {
      let perimeterSum = Array.from(plot)
        .map(p => p.perimeter)
    ...
    return (
      '' +
      Array.from(initialPoints.entries())
        .map(value => value[0] + '=' + value[1])
        .join('|') +
      '</br>' +
      Array.from(initialPoints.values()).reduce((agg, cur) => agg + cur, 0)
    );
  },
  ...
  compute2() {
    ...
    garden.forEach((plot, key) => {
      ...
      let perimeterSum = plotArr
        .map(p => calculateCorners([...plotArr], p, lines.length - 1, lines[0].length - 1))
        .reduce((agg, cur) => agg + cur, 0);
      initialPoints.set(key, perimeterSum * plot.size);
    });
...
function calculateCorners(plotArr: Plot[], point: Plot, maxY: number, maxX: number): number {
  return (
    isTopLeftCornerOut(plotArr, point) +
    isTopRightCornerOut(plotArr, point, maxX) +
    isBottomLeftCornerOut(plotArr, point, maxY) +
    isBottomRightCornerOut(plotArr, point, maxX, maxY) +
    isTopLeftCornerIn(plotArr, point) +
    isTopRightCornerIn(plotArr, point, maxX) +
    isBottomLeftCornerIn(plotArr, point, maxY) +
    isBottomRightCornerIn(plotArr, point, maxX, maxY)
  );
}
...
function isTopLeftCornerIn(plotArr: Plot[], point: Plot): number {
  if (
    !exists(plotArr, point.x + 1, point.y + 1) &&
    exists(plotArr, point.x + 1, point.y) &&
    exists(plotArr, point.x, point.y + 1)
  ) {
    return 1;
  } else {
    return 0;
